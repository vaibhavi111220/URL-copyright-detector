...
if(mysqli_num_rows($queryform) > 0){
    while($result = mysqli_fetch_array($queryform)){
        ...
        echo '<td>' . htmlspecialchars($result['cname']) . '</td>';
        echo '<td>' . htmlspecialchars($result['urls']) . '</td>';
        echo '<td>' . htmlspecialchars($result['yorn']) . '</td>';
        echo '<td><a href="delete.php?del=' . htmlspecialchars($result['id']) . '"><button class="btn btn-danger">Delete</button></a></td>';
        echo '</tr>';
    }
} else {
